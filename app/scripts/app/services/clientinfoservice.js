...
angular.module("popup.services").factory("ClientInfoService", [
  "PortService",
  function (PortService) {
    ...
    ClientInfoService.prototype.handleCartSettings = function (apiConfig) {
      ...
      if (apiConfig && apiConfig.cartSelectors) {
        angular.forEach(apiConfig.cartSelectors, function (
          selectorObj,
          selectorName
        ) {
    ...
    ClientInfoService.prototype.getRuleStates = function () {
      var self = this;
      return PortService.sendAsyncMessage({
        command: COMMANDS.RULE_STATES,
      }).then(function (message) {
        return message.response.ruleStates;
      });
    };

    ClientInfoService.prototype.editRuleCondition = function (
      ruleId,
      conditionIndex,
      newValue,
      newType
    ...
    ClientInfoService.prototype.runCart = function () {
      var self = this;
      return PortService.sendAsyncMessage({ command: COMMANDS.RUNCART }).then(
        function (message) {
    ...
    ClientInfoService.prototype.enableCobrowse = function () {
      return PortService.sendAsyncMessage({
        command: COMMANDS.ENABLE_COBROWSE,
      });
    };
